...
def test_ast_symbol_import_guard_modules_reuse_shared_import_helper():
    violating_modules: list[str] = []
    for module_path in AST_SYMBOL_IMPORT_GUARD_MODULES:
        ...
        if not imports_symbol_from_module(
            ...
            continue
        if not calls_symbol(module_text, "imports_symbol_from_module"):
            violating_modules.append(module_path)
            continue
        if "def _imports_symbol_from_module" in module_text:
            violating_modules.append(module_path)

    assert violating_modules == []


def test_ast_symbol_import_guard_inventory_stays_in_sync():
    excluded_modules = {
    ...
    assert sorted(AST_SYMBOL_IMPORT_GUARD_MODULES) == discovered_modules
