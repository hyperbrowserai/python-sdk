...
def test_ast_quaternary_import_guard_modules_reuse_shared_helper():
    violating_modules: list[str] = []
    for module_path in AST_QUATERNARY_IMPORT_GUARD_MODULES:
        ...
        if not calls_symbol(
            ...
            violating_modules.append(module_path)
            continue
        if "def _imports_imports_imports_imports_collect_function_sources" in module_text:
            violating_modules.append(module_path)

    assert violating_modules == []


def test_ast_quaternary_import_guard_inventory_stays_in_sync():
    excluded_modules = {
    ...
    assert sorted(AST_QUATERNARY_IMPORT_GUARD_MODULES) == discovered_modules
