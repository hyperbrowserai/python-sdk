...
class ControlPlaneAuthManager:
    ...
    def _handle_refresh_response(
        ...
        if response.status_code >= 400:
            ...
            if isinstance(payload, dict):
                ...
                ) or _normalize_text(_string_value(payload.get("code")))
            if error_code in TERMINAL_OAUTH_REFRESH_ERRORS:
                self._expire_oauth_session()
            message = (
                _normalize_text(
                    _string_value(payload.get("message"))
                    if isinstance(payload, dict)
                    else ""
                )
                or _normalize_text(
                    _string_value(payload.get("error_description"))
                    if isinstance(payload, dict)
                    else ""
                )
                or error_code
    ...
    def _expire_oauth_session(self) -> None:
        _delete_oauth_session(
            Path(str(self._mode["session_path"])),
            Path(str(self._mode["lock_path"])),
        )
...
def resolve_control_plane_config(
    ...
    if not _oauth_base_urls_match(session.get("base_url"), resolved_base_url):
        raise HyperbrowserError(
            f"Saved OAuth session for profile {profile} targets {_normalize_base_url(session.get('base_url'))}, not {resolved_base_url}",
...
def _delete_oauth_session(session_path: Path, lock_path: Optional[Path] = None) -> None:
    try:
