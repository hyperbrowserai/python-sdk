...
- Retry classification accepts integer, ASCII numeric-string, and ASCII numeric byte-string HTTP status metadata (including bytes-like inputs such as `memoryview`) when evaluating retryability (including wait-helper status/fetch paths); malformed/oversized/non-ASCII values safely fall back to retryable unknown behavior.
